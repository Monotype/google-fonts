...
node('fonttools-dev') {
    ...
    stage('Update Git Clone') {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'jenkins-github', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_TOKEN']]) {
            ...
            if (true || params.build_type == 'full') {
                files = sh(script: "find . -iregex '.*\\(\\.ttf\\|\\.cff\\|\\.otf\\)\$' | grep noto/ | grep -v CJK | cut -c3- | head -n 1", returnStdout: true).split("\n")
            } else {
    ...
    stage('Create Font Reports') {
        ...
        parallel(stages)
        sh """
            docker run --user \$(id -u):\$(id -g) --rm -v \"\$PWD\":/work -w /work docker-artifact.monotype.com/fonttools/fonttoolkit:latest dump-meta-data --sf --rs=True noto noto/_summary_report.csv
            export GIT_ASKPASS=\$PWD/.git-askpass
